	...
	for app in apps_list.items:
		...
		if APP_NAME != "kubernetes":
			...
			for pod in pod_list.items:
				...
				QUERY_USAGE_cpu_percentage = "%.2f" %((float(CPU_USAGE) / (float (total_limit_cpu)/ 1000))*100)
			        dynamic['CPU usage'] =  str ("%.2f" % float(CPU_USAGE)) + " (" + str(QUERY_USAGE_cpu_percentage) + "%)"
				...
				MEMORY_USAGE = get_query_result(QUERY_USAGE_memory)[0].get('value')[1]
			        QUERY_USAGE_memory_percentage = "%.2f" % float(( float(MEMORY_USAGE) / float(total_limit_mem)) * 100)
